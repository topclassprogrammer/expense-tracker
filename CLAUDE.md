...
Каркас монорепозитория: слой данных и API рабочие, UI — заготовки страниц (формы создания операции и категории помечены `TODO`, графики не подключены). Страницы `/login` и `/register` реализованы.
