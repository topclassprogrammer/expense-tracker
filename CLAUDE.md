...
Каркас монорепозитория: слой данных и API рабочие, UI — заготовки страниц (формы создания расхода и категории помечены `TODO`, графики не подключены).

Окружение уже поднято: зависимости установлены, миграция `init` применена, сид системных категорий выполнен, контейнер PostgreSQL запущен. API проверен сквозным сценарием (регистрация → логин → CRUD расходов → сводка → ротация refresh-токена).
...
- `amountSchema` в `expense.schema.ts` — принимает строку/число и нормализует;
